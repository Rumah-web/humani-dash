...
const Menu = () => {
	...
	const runTabTotal = async (filter: any) => {
		const req = await fetch("/menu-category/api/total-per-status", {
			method: "POST",
	...
	const runTotal = async (where: {}) => {
		const req = await fetch("/menu-category/api/total", {
			method: "POST",
			body: JSON.stringify({
				where,
			}),
	...
	const runQuery = async (
		...
		const req = await fetch("/menu-category/api/list", {
			method: "POST",
			body: JSON.stringify({
				where,
				take,
	...
	const onAdd = async () => {
		const req = await fetch("/menu-category/api/add", {
			method: "POST",
			body: JSON.stringify({}),
			headers: {
				"content-type": "application/json",
		...
		if (req) {
			const { data } = await req.json();
			router.push(`/menu-category/form/${data.uuid}`);
		}
	};

	const onRowClicked = (row: any, event: any) => {
		router.push(`/menu-category/form/${row.uuid}`);
	};
	...
	return (
		<>
			<Breadcrumb pageName='Menu Category' />
			<div className='pb-36'>
				<>
					<div id='header'>
								...
								<div className='flex w-full items-center relative flex-wrap'>
									...
									<div className='flex w-1/2 px-4'>
										<Select
											...
											isClearable={true}
											isSearchable={true}
											onChange={(e) => handleChangeSelect("parent_id", e)}
										/>
									</div>
								</div>
							</form>
